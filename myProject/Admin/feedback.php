...
include('../Admin/Adminheader.php'); include('../dbConnection.php');
 if(isset($_SESSION['is_admin_login'])){
  ...
  <div class="col-sm-9 mt-5">
    <!--Table-->
    <p class=" bg-dark text-white p-2 text-center">List of Feedbacks</p><?php
      ...
      if($result->num_rows > 0){
       echo '<table class="table"><thead><tr>
        ...
        while($row = $result->fetch_assoc()){   echo '<tr>';
         ...
         </tr>';}   echo '</tbody></table>'; } else { echo "0 Result";}
      if(isset($_REQUEST['delete'])){
       ...
       if($conn->query($sql) === TRUE){
         ...
         echo '<meta http-equiv="refresh" content= "0;URL=?deleted" />';} 
           ...
           </div></div>  <!-- div Row close from header --></div>  <!-- div Conatiner-fluid close from header -->
