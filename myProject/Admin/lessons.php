...
define('TITLE', 'Lessons'); include('../Admin/Adminheader.php');  include('../dbConnection.php');
...
<div class="col-sm-9 mt-5  mx-3">
  <form action="" class="mt-3 form-inline d-print-none">
    <div class="form-group mr-3">
      ...
      <input type="text" class="form-control ml-3" id="checkid" name="checkid" onkeypress="isInputNumber(event)" placeholder="Enter Course Id"></div><button type="submit" class="btn btn-danger">Search</button></form><?php
  ...
  while($row = $result->fetch_assoc()){
    if(isset($_REQUEST['checkid']) && $_REQUEST['checkid'] == $row['course_id']){
      ...
      if(($row['course_id']) == $_REQUEST['checkid']){ 
        ...
        <?php $sql = "SELECT * FROM lesson WHERE course_id = {$_REQUEST['checkid']}";
          $result = $conn->query($sql);
        ...
        if(isset($_REQUEST['delete'])){
         ...
         if($conn->query($sql) === TRUE){
           ...
           echo '<meta http-equiv="refresh" content= "0;URL=?deleted" />';}
            else {echo "Unable to Delete Data";} }}}?></div>
